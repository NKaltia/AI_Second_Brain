...
export class NoteService {
    ...
    async addNote(title: string, content: string, tags: string[]): Promise<Note> {
        ...
        const result = await prisma.$queryRaw<Note[]>`
            INSERT INTO "Note" (id, title, content, embedding, tags, "createdAt")
            VALUES (
                gen_random_uuid(),
                ${title},
                ${content},
    ...
    async getAllNotes(): Promise<Note[]> {
        const notesArray = await prisma.note.findMany({ orderBy: { createdAt: 'desc' } });
        return notesArray;
    }
    ...
    async searchSimilarNotes(query: string): Promise<Note[]> {
        ...
        FROM "Note"
        WHERE embedding <=> ${queryVectorString}::vector < 0.4
